...
export default function SwapDashboard({
  ...
  const SWAP_STATUS_LABELS: Record<string, string> = {
    PENDING_REPLACEMENT: t.swap.waitingReplacement,
    PENDING_MANAGER: t.swap.waitingManager,
    APPROVED: t.swap.accepted,
    REJECTED_REPLACEMENT: t.swap.rejectedReplacement,
    REJECTED_MANAGER: t.swap.rejectedManager,
    CANCELLED: t.swap.cancelled,
  };

  const STATUS_FILTERS = [
    { value: "ALL", label: t.swap.all },
    { value: "PENDING_REPLACEMENT", label: t.swap.waitingReplacement },
    { value: "PENDING_MANAGER", label: t.swap.waitingManager },
    { value: "APPROVED", label: t.swap.accepted },
    { value: "REJECTED_REPLACEMENT", label: t.swap.reject },
    { value: "CANCELLED", label: t.swap.cancelled },
  ];
  ...
  const handleCreate = () => {
    if (!selectedShift || !selectedReplacement) {
      showToast(t.swap.selectShift + " + " + t.swap.selectReplacement, "error");
      return;
    }
    startTransition(async () => {
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="page-container pt-4 pb-1 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-extrabold text-foreground">{t.swap.title}</h1>
          <p className="text-xs text-zinc-400 mt-0.5">
            {pendingCount > 0
              ? `${pendingCount} ${t.swap.pendingYour}`
              : t.swap.noPending}
          </p>
        </div>
        <button
        ...
        >
          ...
          <span className="hidden sm:inline">{t.swap.newRequest}</span>
          <span className="sm:hidden">{t.swap.new}</span>
        </button>
      </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    <option value="">— {t.swap.chooseShift} —</option>
                    {myShifts.map((s) => (
                ...
                {myShifts.length === 0 && (
                  <p className="mt-2 text-xs text-zinc-400">{t.swap.noUpcoming}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full text-sm border border-zinc-200 rounded-xl px-4 py-3 bg-zinc-50 text-foreground appearance-none focus:ring-2 focus:ring-brand-purple focus:border-transparent outline-none"
                  >
                    <option value="">— {t.swap.chooseEmployee} —</option>
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-500 mb-1.5">
                  {t.swap.reason} <span className="text-zinc-400 font-normal">({t.swap.optional})</span>
                </label>
                <textarea
                  value={formReason}
                  ...
                  <div className="px-4 py-3.5 flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {isRequester && (
                          <span className="text-xs text-zinc-400">{t.swap.yourRequest}</span>
                        )}
                        {isReplacement && (
                          <span className="text-xs text-brand-purple font-semibold">
                            {t.swap.youAreReplacement}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="mt-3 flex items-center gap-3 text-xs">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-muted">
                            {t.swap.requester}: <strong>{swap.requesterName}</strong>
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-muted">
                            {t.swap.replacement}: <strong>{swap.replacementName}</strong>
                          </span>
                        </div>
                      </div>
                      ...
                      {swap.replacementNote && (
                        <div className="mt-1 text-xs text-zinc-400">
                          {t.swap.replacementNote}: {swap.replacementNote}
                        </div>
                      )}
                      {swap.managerNote && (
                        <div className="mt-1 text-xs text-zinc-400">
                          {t.swap.managerNote}: {swap.managerNote}
                        </div>
                      )}
                      {swap.managerName && (
                        <div className="mt-1 text-xs text-zinc-400">
                          {t.swap.reviewer}: {swap.managerName}
                        </div>
                      )}
                    </div>
                  </div>
